...
function draw() {
  ...
  // tieSound.play()
    ...
    if(keyDown("RIGHT_ARROW") && luke_X_wing.x<330){
      luke_X_wing.x+=5
      }
   
      if(keyDown("LEFT_ARROW") && luke_X_wing.x>90){
      luke_X_wing.x-=5
     }
  ...
  if(bg.y>1000){
    ...
    textSize(12);
      fill ("yellow")
      text("Destroy clicking the s key and shooting into the 2 black circles ",40,200);
      box1.visible = false
      box2.visible = false
  }
   if(b1Group.isTouching(box1)&& b1Group.isTouching(box2)){
      ...
      textSize(15);
      fill ("green")
      text("Death Star is Destroyed the rebellion will live to fight another day",10,250);
    }
}   
